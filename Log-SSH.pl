...
=head2 DESCRIPTION
  ...
  This script is intended as a wrapper for OpenSSH

=head2 INPUTS

  All switches that OpenSSH normally uses as of v7.2p2

=head2 OUTPUTS 

  Executes ssh session and outputs as normal
  Writes compressed log file to ~/logs/<Hostname>.<DateTime>.log.bz2

=head2 LINK

 https://github.com/aaronmcdavid/Log-SSH
...
=head2 NOTES
  ...
  Current Version Date:  2016-04-27
  Purpose/Change:        Added syntactic support for all ssh switches
                         Added bzip compression for logs
...
my $logfile = $logdir . "/${target}.${now}.log" ;
